...
class PageDiscoverer:
    ...
    async def discover(self) -> Optional[dict]:
        ...
        try:
            ...
            # Use real interactive element texts as fingerprint keys. Synthetic
            # nodes do not need to be excluded separately here because
            # _collect_interactive now skips the WebPage root emitted by the
            # WebStateProvider. Keeping the key list short keeps the cache
            # fast-path checks cheap and robust.
            key_texts = [
                (el.get("text", "") or "")[:30]
    ...
    def _collect_interactive(self) -> list[dict]:
        ...
        for el in flat:
            ...
            if el.get("index") is None and not el.get("text"):
                continue
            # Skip synthetic root nodes emitted by WebStateProvider. The
            # "WebPage" root carries text and an arbitrary index but is not a
            # real DOM element, so its text must not be used as a cache key.
            if (el.get("className") or "").lower() == "webpage":
                continue
            out.append(el)
